...
router.post('/register', async (req, res) => {
  try {
    ...
    // Límite cuentas por IP (configurable desde admin panel, default 20)
    let maxAccountsPerIp = 20;
    try {
      const [[cfg]] = await db.execute(
        "SELECT value FROM web_config WHERE `key` = 'max_accounts_per_ip'"
      );
      if (cfg) maxAccountsPerIp = parseInt(cfg.value) || 20;
    } catch { /* usa default */ }
    ...
    if (ipCount >= maxAccountsPerIp)
      return res.status(403).json({
        error: `Límite de cuentas alcanzado para tu IP (máximo ${maxAccountsPerIp}). Contacta al staff.`
      });
