...
/* ──────────────────────────────────────────────────────────────────
   ...
   GET  /api/admin/server-config — Lee configuraciones editables
   PUT  /api/admin/server-config — Actualiza configuraciones
─────────────────────────────────────────────────────────────────── */
router.get('/server-config', auth, adminOnly, async (req, res) => {
  try {
    const [rows] = await db.execute(
      "SELECT `key`, value FROM web_config WHERE `key` IN ('max_accounts_per_ip')"
    );
    const cfg = {};
    rows.forEach(r => { cfg[r.key] = r.value; });
    res.json({
      max_accounts_per_ip: parseInt(cfg['max_accounts_per_ip'] || '20')
    });
  } catch (err) {
    console.error('[admin/server-config GET]', err);
    res.status(500).json({ error: 'Error al obtener configuración' });
  }
});

router.put('/server-config', auth, adminOnly, async (req, res) => {
  const { max_accounts_per_ip } = req.body;

  const val = parseInt(max_accounts_per_ip);
  if (isNaN(val) || val < 1 || val > 999)
    return res.status(400).json({ error: 'max_accounts_per_ip debe ser entre 1 y 999' });

  try {
    await db.execute(
      "INSERT INTO web_config (`key`, value) VALUES ('max_accounts_per_ip', ?) ON DUPLICATE KEY UPDATE value = ?",
      [String(val), String(val)]
    );
    res.json({ ok: true, max_accounts_per_ip: val });
  } catch (err) {
    console.error('[admin/server-config PUT]', err);
    res.status(500).json({ error: 'Error al guardar configuración' });
  }
});
