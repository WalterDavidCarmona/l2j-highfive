...
class L2Api {
  ...
  updateNews(id, data) {
    return this._fetch(`/news/${id}`, { method: 'PUT', body: JSON.stringify(data) });
  }
  deleteNews(id) {
    return this._fetch(`/news/${id}`, { method: 'DELETE' });
  }
  adminGetAllNews(limit = 50, offset = 0) {
    return this._fetch(`/news/admin/all?limit=${limit}&offset=${offset}`);
  }
  async uploadNewsImages(files) {
    ...
    const headers = {};
    if (this._token) headers['Authorization'] = `Bearer ${this._token}`;
    const res = await fetch(API_BASE + '/news/upload', { method: 'POST', headers, body: formData });
    const data = await res.json().catch(() => ({ error: 'Respuesta invalida' }));
    if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
    return data;
  }
  ...
  // ── Admin Server Config ───────────────────────────────────────────
  adminGetServerConfig()           { return this._fetch('/admin/server-config'); }
  adminSetServerConfig(data)       { return this._fetch('/admin/server-config', { method:'PUT', body: JSON.stringify(data) }); }
