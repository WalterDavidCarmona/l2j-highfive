...
/** Parsea image_url que puede ser: null, string URL, o JSON array de URLs */
function parseNewsImages(val) {
  if (!val) return [];
  try {
...
function renderNewsCard(item) {
  ...
  const imgs = parseNewsImages(item.image_url);
  const hasImg = imgs.length > 0;
  return `
    <div class="news-img${hasImg ? ' has-image' : ''}">
      ${hasImg
        ? `<img src="${escHtml(imgs[0])}" alt="${escHtml(item.title)}">`
        : (typeIcons[item.type] || '📰')}
      <span class="news-type-badge ${item.type}">${item.type}</span>
      ${imgs.length > 1 ? `<span class="news-img-count">📷 ${imgs.length}</span>` : ''}
    </div>
    <div class="news-body">
...
function renderNewsSideItem(item) {
  const typeEmojis = { news:'📰', event:'⚔️', update:'🔧', maintenance:'🛠️' };
  const imgs = parseNewsImages(item.image_url);
  const sideImg = imgs.length > 0;
  return `
    <div class="news-side-icon${sideImg ? ' has-thumb' : ''}">${sideImg
      ? `<img src="${escHtml(imgs[0])}" alt="">`
      : (typeEmojis[item.type]||'📰')}</div>
    <div>
...
async function loadNews(reset = true) {
  ...
  const filterType = window.currentNewsFilter || '';
  ...
  try {
    const res = await api.getNews(NEWS_LIMIT, newsOffset, filterType);
    ...
    items.forEach((item, i) => {
      ...
      el.innerHTML = renderNewsCard(item);
      el.addEventListener('click', () => openNewsDetail(item));
      mainGrid.appendChild(el);

      if (reset && i < 5 && sideList) {
        const sideEl = document.createElement('div');
        sideEl.className = 'news-side-item';
        sideEl.innerHTML = renderNewsSideItem(item);
        sideEl.addEventListener('click', () => openNewsDetail(item));
...
/* ── Popup de detalle de noticia ── */
function openNewsDetail(item) {
  document.getElementById('news-detail-overlay')?.remove();

  const typeLabels = { news:'Noticia', event:'Evento', update:'Actualización', maintenance:'Mantenimiento' };
  const typeIcons = { news:'📰', event:'⚔️', update:'🔧', maintenance:'🛠️' };
  const imgs = parseNewsImages(item.image_url);
  const hasImg = imgs.length > 0;
  ...
  if (hasImg && imgs.length === 1) {
    bannerHTML = `<div class="news-detail-banner"><img src="${escHtml(imgs[0])}" alt="${escHtml(item.title)}"><span class="news-type-badge ${item.type}">${typeLabels[item.type] || item.type}</span></div>`;
  } else if (hasImg && imgs.length > 1) {
    bannerHTML = `
      <div class="news-slider" id="news-slider">
        <div class="news-slider-track" id="news-slider-track">
          ${imgs.map(u => `<div class="news-slider-slide"><img src="${escHtml(u)}" alt="${escHtml(item.title)}"></div>`).join('')}
        </div>
        <button class="news-slider-btn news-slider-prev" onclick="newsSliderNav(-1)">❮</button>
        <button class="news-slider-btn news-slider-next" onclick="newsSliderNav(1)">❯</button>
        <div class="news-slider-dots" id="news-slider-dots">
          ${imgs.map((_, i) => `<span class="news-slider-dot${i===0?' active':''}" onclick="newsSliderGo(${i})"></span>`).join('')}
        </div>
        <span class="news-type-badge ${item.type}">${typeLabels[item.type] || item.type}</span>
      </div>`;
  } else {
    bannerHTML = `<div class="news-detail-banner news-detail-no-img">${typeIcons[item.type]||'📰'}<span class="news-type-badge ${item.type}">${typeLabels[item.type] || item.type}</span></div>`;
  }
  ...
  overlay.innerHTML = `
    <div class="news-detail-modal">
      <button class="news-detail-close" onclick="this.closest('.news-detail-overlay').remove()">✕</button>
      ${bannerHTML}
      <div class="news-detail-body">
        <h2 class="news-detail-title">${item.pinned ? '📌 ' : ''}${escHtml(item.title)}</h2>
        <div class="news-detail-meta">
          <span>👤 ${escHtml(item.author || 'Admin')}</span>
          <span>📅 ${formatDate(item.created_at)}</span>
          ${item.updated_at && item.updated_at !== item.created_at ? `<span>✏️ ${formatDate(item.updated_at)}</span>` : ''}
        </div>
  ...
  // Init slider state + autoplay
  if (imgs.length > 1) {
    window._newsSliderIdx = 0;
    window._newsSliderTotal = imgs.length;
    newsSliderAutoStart();
  }
}

function newsSliderNav(dir) {
  const idx = (window._newsSliderIdx || 0) + dir;
  const total = window._newsSliderTotal || 1;
  newsSliderGo(((idx % total) + total) % total);
  newsSliderAutoStart(); // reset timer on manual nav
}
function newsSliderGo(idx) {
  window._newsSliderIdx = idx;
  const track = document.getElementById('news-slider-track');
  if (track) track.style.transform = `translateX(-${idx * 100}%)`;
  document.querySelectorAll('#news-slider-dots .news-slider-dot').forEach((d, i) => {
    d.classList.toggle('active', i === idx);
  });
  newsSliderAutoStart(); // reset timer on dot click
}
function newsSliderAutoStart() {
  clearInterval(window._newsSliderTimer);
  window._newsSliderTimer = setInterval(() => {
    if (!document.getElementById('news-slider')) { clearInterval(window._newsSliderTimer); return; }
    const next = ((window._newsSliderIdx || 0) + 1) % (window._newsSliderTotal || 1);
    window._newsSliderIdx = next;
    const track = document.getElementById('news-slider-track');
...
window.newsSliderNav  = newsSliderNav;
window.newsSliderGo   = newsSliderGo;
...
function loadAdmin() {
  ...
  document.getElementById('admin-tab-news')?.classList.add('hidden');
  document.getElementById('admin-tab-serverconfig')?.classList.add('hidden');
}
...
document.querySelectorAll('[data-admin-tab]').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    document.getElementById('admin-tab-news')?.classList.toggle('hidden', tab !== 'news');
    document.getElementById('admin-tab-serverconfig')?.classList.toggle('hidden', tab !== 'serverconfig');
    if (tab === 'news') {
      loadAdminNews();
    }
    if (tab === 'payments') {
...
async function loadAdminNews() {
  ...
  if (!tbody) return;
  tbody.innerHTML = '<tr><td colspan="9" class="text-center"><div class="spinner" style="margin:1rem auto"></div></td></tr>';

  try {
    const res = await api.adminGetAllNews(100, 0);
    const items = res.items || [];
    if (!items.length) {
      tbody.innerHTML = '<tr><td colspan="9" class="text-center text-muted">No hay noticias</td></tr>';
      return;
    }

    const typeLabels = { news:'📰 Noticia', event:'⚔️ Evento', update:'🔧 Update', maintenance:'🛠️ Mant.' };

    tbody.innerHTML = items.map(n => `
      <tr style="opacity:${n.active ? 1 : 0.5}">
        <td>${n.id}</td>
        <td style="max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escHtml(n.title)}</td>
        <td>${typeLabels[n.type] || n.type}</td>
        <td>${escHtml(n.author || '-')}</td>
        <td>${n.image_url ? '<img src="'+escHtml(n.image_url)+'" style="height:32px;border-radius:4px" onerror="this.style.display=\'none\'">' : '—'}</td>
        <td>${n.pinned ? '📌' : ''}</td>
        <td>${n.active ? '<span style="color:var(--green)">Si</span>' : '<span style="color:var(--red)">No</span>'}</td>
        <td style="font-size:.8rem">${formatDate(n.created_at)}</td>
        <td>
          <button class="btn btn-secondary btn-sm" onclick="editNews(${n.id})" style="padding:3px 8px;font-size:.75rem">✏️</button>
          <button class="btn btn-danger btn-sm" onclick="deleteNewsItem(${n.id})" style="padding:3px 8px;font-size:.75rem">🗑</button>
        </td>
      </tr>
    `).join('');
  } catch (err) {
    tbody.innerHTML = `<tr><td colspan="9" class="text-center" style="color:var(--red)">${err.message}</td></tr>`;
  }
}

function openNewsEditor(data) {
  const panel = document.getElementById('news-editor-panel');
  panel.classList.remove('hidden');
  document.getElementById('news-edit-id').value = data?.id || '';
  document.getElementById('news-edit-title').value = data?.title || '';
  document.getElementById('news-edit-type').value = data?.type || 'news';
  document.getElementById('news-edit-pinned').value = data?.pinned ? '1' : '0';
  document.getElementById('news-edit-active').value = data?.active !== false && data?.active !== 0 ? '1' : '0';
  document.getElementById('news-edit-content').value = data?.content || '';
  document.getElementById('news-edit-image-url').value = data?.image_url || '';
  document.getElementById('news-edit-image-file').value = '';
  document.getElementById('news-edit-image-status').textContent = '';
  document.getElementById('news-editor-msg').classList.add('hidden');

  // Parsear imagenes (puede ser JSON array o string simple)
  let imgs = parseNewsImages(data?.image_url);
  if (imgs.length) {
    document.getElementById('news-edit-image-status').textContent = `${imgs.length} imagen(es)`;
  }
  renderImagePreview(imgs);
...
async function editNews(id) {
  try {
    ...
    showToast('Error cargando noticia: ' + err.message, 'error');
  }
}
...
async function uploadNewsImg() {
  const fileInput = document.getElementById('news-edit-image-file');
  const status = document.getElementById('news-edit-image-status');
  const files = fileInput.files;
  if (!files || !files.length) { status.textContent = 'Selecciona imagenes primero'; return; }

  status.textContent = `Subiendo ${files.length} imagen(es)...`;
  try {
    const res = await api.uploadNewsImages(files);
    // Merge con existentes
    let current = [];
    try { current = JSON.parse(document.getElementById('news-edit-image-url').value) || []; } catch {}
    if (!Array.isArray(current)) current = current ? [current] : [];
    const merged = [...current, ...res.urls];
    document.getElementById('news-edit-image-url').value = JSON.stringify(merged);
    renderImagePreview(merged);
    status.textContent = `✅ ${res.urls.length} imagen(es) subida(s) (${merged.length} total)`;
...
function renderImagePreview(urls) {
  const container = document.getElementById('news-edit-image-preview');
  if (!container) return;
  if (!urls || !urls.length) { container.innerHTML = ''; return; }
  container.innerHTML = '<div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-top:.5rem">' +
    urls.map((u, i) => `<div style="position:relative;display:inline-block">
      <img src="${escHtml(u)}" style="height:80px;border-radius:6px;border:1px solid var(--border)" onerror="this.style.display='none'">
      <button onclick="removeNewsImage(${i})" style="position:absolute;top:-6px;right:-6px;width:20px;height:20px;border-radius:50%;background:rgba(255,50,50,.9);border:none;color:#fff;font-size:.7rem;cursor:pointer;display:flex;align-items:center;justify-content:center">✕</button>
    </div>`).join('') + '</div>';
}

function removeNewsImage(index) {
  let urls = [];
  try { urls = JSON.parse(document.getElementById('news-edit-image-url').value) || []; } catch {}
  if (!Array.isArray(urls)) return;
  urls.splice(index, 1);
  document.getElementById('news-edit-image-url').value = urls.length ? JSON.stringify(urls) : '';
  renderImagePreview(urls);
  document.getElementById('news-edit-image-status').textContent = urls.length ? `${urls.length} imagen(es)` : '';
}
...
async function saveNews() {
  ...
  const type     = document.getElementById('news-edit-type').value;
  const pinned   = document.getElementById('news-edit-pinned').value === '1';
  const active   = document.getElementById('news-edit-active').value === '1';
  let   image_url = document.getElementById('news-edit-image-url').value.trim();
  ...
  // Si hay archivos seleccionados sin subir, subirlos automaticamente
  const pendingFiles = document.getElementById('news-edit-image-file')?.files;
  if (pendingFiles && pendingFiles.length && !image_url) {
    try {
      showToast('Subiendo imagenes...', 'info');
      const upRes = await api.uploadNewsImages(pendingFiles);
      image_url = JSON.stringify(upRes.urls);
      document.getElementById('news-edit-image-url').value = image_url;
      renderImagePreview(upRes.urls);
    } catch (err) {
      showToast('Error subiendo imagenes: ' + err.message, 'error');
      return;
    }
  }

  const data = { title, content, type, pinned, active, image_url: image_url || null };
  ...
  try {
    ...
    closeNewsEditor();
    loadAdminNews();
    // Refrescar la seccion publica de noticias tambien
    newsOffset = 0;
  } catch (err) {
    showToast('Error: ' + err.message, 'error');
  }
...
/* ── Toolbar helpers para el editor de noticias ── */
function newsInsertTag(tag) {
  const ta = document.getElementById('news-edit-content');
...
function newsInsertColor() {
  const color = prompt('Color (ej: #ff4444, red, var(--gold)):', '#c8a84b');
  if (!color) return;
  const ta = document.getElementById('news-edit-content');
  const start = ta.selectionStart, end = ta.selectionEnd;
  const selected = ta.value.substring(start, end) || 'texto';
...
function newsInsertImg() {
  const url = prompt('URL de la imagen:', document.getElementById('news-edit-image-url').value || '');
  if (!url) return;
  const ta = document.getElementById('news-edit-content');
  const pos = ta.selectionStart;
  const insert = `<img src="${url}" style="max-width:100%;border-radius:8px;margin:0.5rem 0">`;
...
function updateNewsPreview() {
  const content = document.getElementById('news-edit-content')?.value || '';
  const preview = document.getElementById('news-edit-preview');
  if (preview) preview.innerHTML = content;
}

// Actualizar preview al escribir
document.getElementById('news-edit-content')?.addEventListener('input', updateNewsPreview);

window.openNewsEditor    = openNewsEditor;
window.closeNewsEditor   = closeNewsEditor;
window.editNews          = editNews;
window.deleteNewsItem    = deleteNewsItem;
window.uploadNewsImg     = uploadNewsImg;
window.saveNews          = saveNews;
window.newsInsertTag     = newsInsertTag;
window.newsInsertBr      = newsInsertBr;
window.newsInsertColor   = newsInsertColor;
window.newsInsertImg     = newsInsertImg;
...
async function loadServerConfig() {
  try {
    const data = await api.adminGetServerConfig();
    const input = document.getElementById('scfg-max-accounts');
    if (input) input.value = data.max_accounts_per_ip ?? 20;
  } catch (err) {
    showToast('Error cargando configuración: ' + err.message, 'error');
  }
}
...
async function saveServerConfig() {
  const val = parseInt(document.getElementById('scfg-max-accounts')?.value || '20');
  if (isNaN(val) || val < 1 || val > 999) {
    showToast('El valor debe ser entre 1 y 999', 'error');
    return;
  }

  const btn = document.getElementById('scfg-save-btn');
  if (btn) btn.disabled = true;

  try {
    await api.adminSetServerConfig({ max_accounts_per_ip: val });
    showToast(`✅ Máximo de cuentas por IP actualizado a ${val}`, 'success');
  } catch (err) {
